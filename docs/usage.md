...
| `--modkit_phased`      | A boolean to run `modkit pileup --phased` on the Longphase-haplotagged BAMs, producing `_hp1`, `_hp2` and `_combined` bedMethyl files per sample instead of a single unphased file. Known limitation: for PacBio BAMs that carry 6mA calls, modkit falls back to its general pileup workers, which ignore the haplotype partition (`_hp1`/`_hp2` come out empty) and also emit `h` rows despite `--modified-bases 5mC`; ONT data is unaffected. Default = `false`                                                                                        |
