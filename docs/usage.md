...
For structural variants, the CHM13 panel of normals passed to SEVERUS (`--pon_file`) is a merged panel combining the 1000 Genomes CHM13 panel shipped with SEVERUS and the ASAP cohort, with median confidence intervals per breakpoint. It is downloaded automatically with `--genome CHM13`. GRCh38 continues to use the 1000 Genomes panel shipped with SEVERUS.
...
| `--pon_file`       | Panel of Normals breakpoint table (bgzipped CSV) used by SEVERUS for somatic SV filtering, tumor-only. Auto-populated from `--genome`.       |
