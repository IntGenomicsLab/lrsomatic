process WAKHAN {
    ...
    // Whole per-solution directories, not the plots inside them: every solution's
    // copy-number plot has the same basename, so emitting the files individually
    // would collide when staged downstream. LRSOMATICREPORT resolves each plot by
    // solution_<rank>/ path, so the directory has to survive staging.
