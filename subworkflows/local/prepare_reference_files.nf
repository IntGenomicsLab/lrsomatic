...
workflow PREPARE_REFERENCE_FILES {
    ...
    main:
        ...
        if (!ascat_alleles) allele_files = Channel.empty()
            ...
            allele_files = UNZIP_ALLELES.out.unzipped_archive.flatMap { it[1].listFiles() }
        ...
        if (!ascat_loci) loci_files = Channel.empty()
            ...
            loci_files = UNZIP_LOCI.out.unzipped_archive.flatMap { it[1].listFiles() }
        ...
        if (!ascat_loci_gc) gc_file = Channel.value([])
            ...
            gc_file = UNZIP_GC.out.unzipped_archive.flatMap { it[1].listFiles() }
        ...
        if (!ascat_loci_rt) rt_file = Channel.value([])
            ...
            rt_file = UNZIP_RT.out.unzipped_archive.flatMap { it[1].listFiles() }
