...
- [#191](https://github.com/IntGenomicsLab/lrsomatic/pull/191) - Added ClairS-TO Verdict support for non-GRCh38 references. With `--genome CHM13` the pipeline now assembles the loci, allele, GC and replication-timing resource set that ClairS-TO's Verdict module needs (new `CLAIRSTO_CNA_RESOURCES` module and `PREPARE_CLAIRSTO_CNA` subworkflow), passes it via `--cna_resource_dir`, and publishes it to `<outdir>/cache/clairsto` so later runs can reuse it with the new `--clairsto_cna_resources`. The loci, allele and GC files are the same CHM13 ASCAT files the pipeline already downloads; only the replication-timing track is new (`clairsto_cna_rt` in `conf/igenomes.config`, built by `assets/clairsto_cna_chm13/make_chm13_rt.py`). Also new: `--clairsto_disable_verdict` and `--clairsto_verdict_check` (`error` | `warn` | `off`). The ASCAT reference files are now resolved whenever Verdict needs them, not only when `--skip_ascat` is false. GRCh38 is unaffected: its resources ship inside the ClairS-TO image and the flag is left off (@ljwharbers).
...
- [#191](https://github.com/IntGenomicsLab/lrsomatic/pull/191) - Fixed silent loss of germline tagging on CHM13 in tumor-only mode. ClairS-TO's Verdict module decides whether it can run by intersecting the reference contig names with `chr1..chr22,chrX`; CHM13 uses exactly those names, so the check passes and Verdict then applies its GRCh38 loci, allele, GC and replication-timing files to T2T coordinates. Nothing matches, no variant is tagged, and every failure path inside Verdict exits 0, so the run reports success. On a tumor-only PacBio Revio sample this left 42,931 PASS SNVs on CHM13 against 31,942 on GRCh38, about 8,200 of them high-VAF germline SNVs, and added a spurious SBS40a = 12,077 component to the mutational-signature fit. With matched resources the same sample gives 34,708 PASS SNVs and 8,223 `Verdict_Germline` tags, and the CHM13 signature fit reproduces the GRCh38 one at cosine 0.999. `CLAIRSTO` now also fails by default (`--clairsto_verdict_check`) when Verdict produces no tags, so this cannot pass unnoticed again (@ljwharbers).
- [#191](https://github.com/IntGenomicsLab/lrsomatic/pull/191) - `CLAIRSTO` now runs a patched ClairS-TO v0.5.1 (`ghcr.io/ljwharbers/clairs-to:0.5.1-verdictchm13-<sha>`, and the native SIF `oras://ghcr.io/ljwharbers/clairs-to-sif:0.5.1-verdictchm13-<sha>` under Singularity/Apptainer), up from v0.4.2. The patch fixes an `IndexError` in `src/verdict/run_ascat.py` when a logR segment has no heterozygous SNP within the +/-10000-probe window, which a male chrX hits on CHM13; the file is byte identical from v0.4.2 to upstream HEAD, so the fix has been offered upstream. The bump also brings stream-based panel-of-normals processing, which matters for the five-panel CHM13 PoN set, and the `ont_r10_dorado_hac_5khz_v6` model. Note that from v0.4.4 ClairS-TO renames its output VCFs when `--sample_name` is set, and passing `--snv_output_prefix snv` does not opt out, so the module now asks for non-default prefixes and restores the canonical `snv.vcf.gz` / `indel.vcf.gz` names (@ljwharbers).
- [#191](https://github.com/IntGenomicsLab/lrsomatic/pull/191) - `CLAIRSTO` now passes `--conda_prefix` under `-profile apptainer` as well as `-profile singularity`. The guard tested `workflow.containerEngine == 'singularity'` only, so under Apptainer ClairS-TO fell back to whatever `python` was on the container PATH (@ljwharbers).
